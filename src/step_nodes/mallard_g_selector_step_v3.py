...
def slam_callback(data, paramf):
        ...
        if(back_and_forth):
            ...
            if(counter <= counter_max and n_goals == 1): 
                if(counter % 10 == 0): 
                    ...
                    print(  "Settling for " + str(counter_max/10) + " seconds,counter value: " + str(counter/10) + " seconds")
                n_goals = 0 # decrement n_goals to maitain the position
                counter += 1
    ...
    # print("n_goals to send to controller: ", goal_number)
    # publish goal array
